...
// Provider for shared recipes
final sharedRecipesProvider = FutureProvider<List<Recipe>>((ref) async {
  final repository = ref.watch(recipeRepositoryProvider);
  return await repository.getSharedRecipes();
});

class SharedRecipesPage extends ConsumerWidget {
  const SharedRecipesPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final recipesAsync = ref.watch(sharedRecipesProvider);
  ...
  Widget _buildRecipeList(BuildContext context, List<Recipe> recipes) {
    if (recipes.isEmpty) {
      return const Center(child: Text('공유된 레시피가 없습니다.'));
    }
