...
// Provider for generated recipes
final generatedRecipesProvider = FutureProvider<List<Recipe>>((ref) async {
  ...
  return allRecipes
      .where((recipe) => recipe.tags.contains('generated'))
      .toList();
});

class GeneratedRecipesPage extends ConsumerWidget {
  const GeneratedRecipesPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final recipesAsync = ref.watch(generatedRecipesProvider);

    return Scaffold(
      appBar: AppBar(title: const Text('생성된 레시피')),
  ...
  Widget _buildRecipeList(BuildContext context, List<Recipe> recipes) {
    if (recipes.isEmpty) {
      return const Center(child: Text('생성된 레시피가 없습니다.'));
    }
